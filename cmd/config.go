...
func ParseConfig(path string) (*Config, error) {
...

	{
		if cfg.PrePullImageReconcilerConfig.MainContainer.Name == "" {
			return nil, fmt.Errorf("pre_pull_image_reconciller.main_container.name is empty")
		}
		if cfg.PrePullImageReconcilerConfig.MainContainer.Image == "" {
			return nil, fmt.Errorf("pre_pull_image_reconciller.main_container.image is empty")
		}

		if _, err := cfg.PrePullImageReconcilerConfig.MainContainer.Resources.Requests.ToResourcesList(); err != nil {
			return nil, fmt.Errorf("pre_pull_image_reconciller.main_container.resources.requests cannot be converted into corev1.ResourceList: %w", err)
		}

		if _, err := cfg.PrePullImageReconcilerConfig.MainContainer.Resources.Limits.ToResourcesList(); err != nil {
			return nil, fmt.Errorf("pre_pull_image_reconciller.main_container.resources.limits cannot be converted into corev1.ResourceList: %w", err)
		}

		if _, err := cfg.PrePullImageReconcilerConfig.PrePullContainer.Resources.Requests.ToResourcesList(); err != nil {
			return nil, fmt.Errorf("pre_pull_image_reconciller.pre_pull_container.resources.requests cannot be converted into corev1.ResourceList: %w", err)
		}

		if _, err := cfg.PrePullImageReconcilerConfig.PrePullContainer.Resources.Limits.ToResourcesList(); err != nil {
			return nil, fmt.Errorf("pre_pull_image_reconciller.pre_pull_container.resources.limits cannot be converted into corev1.ResourceList: %w", err)
		}
	}
